-- ════════════════════════════════════════════════════════════════════════════
-- Migration 040 — Task 7: covering indexes on unindexed foreign key columns
-- ════════════════════════════════════════════════════════════════════════════
--
...
BEGIN
  ...
  IF array_length(missing, 1) > 0 THEN
    RAISE EXCEPTION '[040] VERIFICATION FAILED: missing indexes: %', array_to_string(missing, ', ');
  END IF;

  RAISE NOTICE '[040] OK: 12 FK covering indexes created ✓';
END $$;
