-- ════════════════════════════════════════════════════════════════════════════
-- Migration 035 — Task 4: restrict corridor-covers storage bucket
-- ════════════════════════════════════════════════════════════════════════════
--
-- CONTEXT:
...
BEGIN
  ...
  IF is_public IS NULL THEN
    RAISE EXCEPTION '[035] VERIFICATION FAILED: corridor-covers bucket not found';
  END IF;
  IF is_public THEN
    RAISE EXCEPTION '[035] VERIFICATION FAILED: corridor-covers bucket is still public=TRUE';
  END IF;
  ...
  IF EXISTS (
    ...
    RAISE EXCEPTION '[035] VERIFICATION FAILED: open SELECT policy corridor_covers_select_public still exists';
  END IF;

  RAISE NOTICE '[035] OK: corridor-covers bucket is private; open SELECT policy removed; authenticated-only policies active';
END $$;
