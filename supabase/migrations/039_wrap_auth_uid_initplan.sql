-- ════════════════════════════════════════════════════════════════════════════
-- Migration 039 — Task 6: wrap bare auth.uid() in (select auth.uid()) for
...
BEGIN
  ...
  IF NOT EXISTS (
    ...
    RAISE EXCEPTION '[039] VERIFICATION FAILED: committed_is_admin() function not found';
  END IF;
  ...
  IF chk_clause IS NULL OR chk_clause NOT LIKE '%committed_is_admin%' THEN
    RAISE EXCEPTION '[039] VERIFICATION FAILED: profiles_update_own does not use committed_is_admin(). Got: %', chk_clause;
  END IF;
  ...
  IF bare_count < 5 THEN
    RAISE EXCEPTION '[039] VERIFICATION FAILED: only %/5 rewritten policies found', bare_count;
  END IF;

  RAISE NOTICE '[039] OK: committed_is_admin() created; 5 policies rewritten; check_ins_player_view updated';
END $$;
