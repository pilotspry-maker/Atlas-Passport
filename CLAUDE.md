...
| 020 | `020_idempotent_seed_helpers.sql` | PR #27 follow-up: redeploy `create_exploit_test_users()` with the Clean slate DELETE block + defensive `profiles` backfill; backfill `profiles` inside `seed_ci_passports()` before passport INSERT (fixes 23503 FK violation) | ✅ applied |
| 021 | `021_fix_seed_phone_collision.sql` | Fix 23505 `users_phone_key` self-collision in `create_exploit_test_users()` by inserting `phone` and `phone_change` as NULL (not `''`); skip both in the dynamic NULL-fix loop | ✅ applied |
| 022 | `022_align_seed_ci_passports_uuids.sql` | Align `seed_ci_passports()` corridor/node UUIDs to the actual deployed `00000000-...` family used by `seed_ci_fixtures`; make it self-sufficient by upserting corridor + node before passport INSERT; fill NOT NULL `activated_at` / `expires_at` | ✅ applied |
| 023 | `023_seed_clean_slate_child_rows.sql` | Extend `create_exploit_test_users()` Clean slate to delete `check_ins` then `passports` before `auth.identities` → `auth.users`, fixing 23503 on `check_ins_user_id_fkey` from prior CI runs | ✅ applied |
| 024 | `024_seed_user_null_fix_and_corridor_slug.sql` | Add dynamic NULL catch-all (excluding `phone` / `phone_change`) to `create_test_users()` and `create_regression_users()` so `email_change` etc. backfill to `''` (fixes GoTrue 500 `unexpected_failure`); redeploy `seed_regression_passports()` with NOT NULL `slug` + `city` supplied | ✅ applied |
| 025 | `025_exploit_test_users_phone_change_fix.sql` | Redeploy `create_exploit_test_users()` with inline `phone_change = ''` and dynamic catch-all excluding only `phone` (not `phone_change`), fixing GoTrue user-list scan error `converting NULL to string is unsupported` | ✅ applied |
| 026 | `026_drop_legacy_public_read_corridors_nodes.sql` | Drop legacy `Public read corridors` / `Public read nodes` USING(true) PUBLIC policies that OR-merged with the authenticated-only policies and let anon enumerate every corridor + node via PostgREST joins | ✅ applied |
| 027 | `027_align_seed_ci_fixtures_and_gate_reward_reads.sql` | Migrate CI fixtures off legacy `00000000-...` UUIDs onto `aaaaaaaa/bbbbbbbb/cccccccc-...` family expected by `tests/rls/test_rls_policies.py`; set `redemption_code = SECRET-CODE-XYZ`; tighten `rewards_select_auth` to require a complete passport on the reward's corridor | ✅ applied |
| 028 | `028_fix_regression_user_passwords.sql` | Restore `RegPlayer1!RLS` / `RegPlayer2!RLS` passwords on regression users (migration 024 churned them, breaking the regression suite with 400 `invalid_credentials`); reset bcrypt hashes on live `auth.users` rows | ✅ applied |
| 029 | `029_fix_regression_users_clean_slate.sql` | Extend `create_regression_users()` Clean slate to delete `public.check_ins` and `public.passports` before `auth.identities` / `auth.users`, fixing 23503 on `check_ins_user_id_fkey` (mirror of 023 for the regression UUIDs) | ✅ applied |
| 030 | `030_rls_audit_helpers.sql` | `get_public_rls_policies()` and `get_public_rls_status()` SECURITY DEFINER RPCs exposing `pg_policies` / `pg_class.relrowsecurity` for the public schema so REG-4 policy-inventory tests can query them via PostgREST | ✅ applied |
| 031 | `031_fix_rewards_policy_checkins_grant.sql` | Rename `rewards_select_auth` → `rewards_select_own` (REG-4a); rescope legacy `Travelers insert own checkins` policy TO authenticated (REG-4f); reaffirm `confirm_test_users()` service_role-only EXECUTE grant | ✅ applied |
| 032 | `032_orion_schema_baseline.sql` | Baseline 5 out-of-band Orion tables (`traveler_profiles`, `passport_activations`, `mission_progress`, `ap_events`, `referral_events`) + 12 policies into the repo so CI can bootstrap from zero; no-op on live via `CREATE TABLE IF NOT EXISTS` / `DROP POLICY IF EXISTS` | ✅ applied |
| 033 | `033_rescope_legacy_travelers_policies.sql` | Rescope 15 RLS policies TO authenticated and wrap `auth.uid()` as `(select auth.uid())` for initplan caching: 11 legacy `Travelers *` policies on Orion tables + 4 repo-owned SELECT policies from migration 001 (`check_ins_select_own`, `passports_select_own`, `profiles_select_own`) | ✅ applied |
| 034 | `034_tighten_service_policies_and_public_bucket.sql` | Restrict `Service inserts *` policies on `ap_events`/`referral_events` to service_role; rescope `Public join waitlist` TO anon+authenticated; add explicit deny-all policies on `corridor.audit_log` and `corridor.jobs`; tighten `corridor-covers` storage SELECT to drop bucket listing; revoke `get_public_stats()` EXECUTE from anon/authenticated (view path replaces it) | ✅ applied |
| 035 | `035_fix_profiles_update_own_recursion.sql` | Replace inline sub-SELECT in `profiles_update_own` WITH CHECK (caused 42P17 infinite recursion / HTTP 500) with `current_user_is_admin()` SECURITY DEFINER helper; restores expected 403/42501 contract for is_admin self-escalation block | ✅ applied |
| 036 | `036_seed_ci_passports_reset_reward_claimed.sql` | Bracket `seed_ci_passports()` upsert with `SET LOCAL session_replication_role = replica` to bypass the `prevent_reward_unclaim` trigger inside the function only; makes the seed truly idempotent for `reward_claimed = false` between exploit-03 runs | ✅ applied |
...
| 042 | `042_passports_update_guard.sql` | Add explicit `passports_update_own` UPDATE policy for authenticated users — locks user_id, corridor_id, expires_at, and activated_at as immutable; blocks anon/public UPDATE entirely (the 2026-06-29 exploit class) | ✅ applied (2026-06-30, PR #82) |

**Status as of 2026-06-30:** All migrations 001–042 are applied to production, verified by direct introspection of `pg_proc`, `pg_policies`, `pg_trigger`, `information_schema.columns`, and `has_function_privilege(service_role, ...)` against project `gaavynmmysdhovpatzlp`. The 2026-06-27 → 2026-06-29 launch-protection sweep applied 018–036; PR #53 (2026-06-29) added 037–040; PR #65 added 041; PR #82 added 042. Migrations 004–5, 007, 009–10, 013–17 were marked PENDING in earlier revisions of this document but were already live — the markers were stale, not the migrations.
