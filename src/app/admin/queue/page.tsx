...
export default async function AdminQueuePage() {
  ...
  const { data: checkIns } = await admin
    ...
    .select(`
      *,
      node:nodes(name, sequence, corridor:corridors(name, city)),
      profile:profiles(email, full_name),
      passport:passports(activated_at, expires_at)
    `)
    .eq('status', 'pending')
    ...
    <div>
      ...
      {!checkIns?.length ? (
        ...
        <div className="space-y-3">
          {checkIns.map(ci => {
            const node = ci.node as { name: string; sequence: number; corridor: { name: string; city: string } } | null
            const profile = ci.profile as { email: string; full_name: string | null } | null
            const passport = ci.passport as { activated_at: string; expires_at: string } | null
            const isExpiringSoon = passport
              ? new Date(passport.expires_at).getTime() - Date.now() < 6 * 60 * 60 * 1000
              : false
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1">
                    <span className="text-xs font-mono text-atlas-gold">#{node?.sequence}</span>
                    <span className="font-semibold text-atlas-text truncate">{node?.name}</span>
                    {isExpiringSoon && (
                      <span className="text-xs border border-atlas-red text-atlas-red px-1 flex-shrink-0">EXPIRING</span>
                    )}
                  </div>
                  <p className="text-xs text-atlas-muted truncate">{node?.corridor?.name} — {node?.corridor?.city}</p>
                  <p className="text-xs text-atlas-text-dim mt-0.5 truncate">
                    {profile?.full_name ?? profile?.email}
                  </p>
                  {ci.notes && (
                    <p className="text-xs text-atlas-muted mt-1 truncate italic">
                      &quot;{ci.notes}&quot;
                    </p>
                  )}
                </div>
